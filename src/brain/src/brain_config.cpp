...
void BrainConfig::calcMapMarkings() {
    ...
    // Four points on the halfway line.
    for (auto side: sides) {
        double ySign = side == "L"? 1.0 : -1.0;
...
{
    // playerStartPos [left, right] controls the side used by enter_field.
    ...
    if (teamId < 0 || teamId > UINT8_MAX)
    {
        throw invalid_argument("[Error] team_id must be in [0, 255]. Got: " + to_string(teamId));
    }

    if (numOfPlayers < 1 || numOfPlayers > MAX_NUM_PLAYERS)
    {
        throw invalid_argument("[Error] number_of_players must be in [1, 20]. Got: " + to_string(numOfPlayers));
    }
    ...
    if (playerId < 1 || playerId > numOfPlayers)
    {
        throw invalid_argument(
            "[Error] player_id must be in [1, number_of_players]. Got: " +
            to_string(playerId));
    }
...
void BrainConfig::print(ostream &os)
{
    ...
    os << "    robotOdomThetaOffset = " << robotOdomThetaOffset << endl;
    os << "    robotOdomThetaAutoAlign = " << robotOdomThetaAutoAlign << endl;
    os << "    robotOdomThetaAlignmentDistance = "
       << robotOdomThetaAlignmentDistance << endl;
    os << "    robotOdomThetaAlignmentMinConcentration = "
       << robotOdomThetaAlignmentMinConcentration << endl;
    ...
    os << "    visualHz = " << rerunLogVisualHz << endl;
    os << "    timeseriesHz = " << rerunLogTimeseriesHz << endl;
    os << "    debugHz = " << rerunLogDebugHz << endl;
    os << "----------------------------------------" << endl;
    os << "OdomLog:" << endl;
    os << "    enable = " << odomLogEnable << endl;
    os << "    logDir = " << odomLogDir << endl;
    os << "    hz = " << odomLogHz << endl;
    os << "    flushIntervalMs = " << odomLogFlushIntervalMs << endl;
    os << "----------------------------------------" << endl;
    os << "Sound:" << endl;
