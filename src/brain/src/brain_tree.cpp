...
NodeStatus DefenderDecide::tick() {
    double chaseThreshold = 1.0;
    getInput("chase_threshold", chaseThreshold);
    string lastDecision;
    getInput("decision_in", lastDecision);

    // ponytail: local computation off the already team-synced ball position
    // (handleCooperation's trustedTMIdx merge into brain->data->ball.posToField,
    // brain.cpp) instead of a new team-broadcast field. Own-half convention
    // matches the rest of the codebase: own goal is always -x, no halftime flip.
    bool ballCurrentOnOurHalf = brain->data->ball.posToField.x < 0.0;
    ...
    // possible" -- defaults are the same ones each get_parameter call below
    ...
    // Every state/gate value the ladder above touches, in one place -- both
    // the console (via cout, greppable in brain.log like [StrikerDecide]/
    // [GoalieDecide]) and Rerun (via logToScreen) get the same complete line.
    const string debugLine = format(
    ...
    std::cout << "[DefenderDecide] " << debugLine << std::endl;

    brain->log->logToScreen("tree/Decide", debugLine, 0xFFFFFFFF);
    return NodeStatus::SUCCESS;
}
